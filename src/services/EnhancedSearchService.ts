...
function testSuiteMatchGenerator(query: any, prefix: string = "") {
    ...
    const compilationModeMatch = query.compilation_mode.length ? { [prefix + "metaData.compilation_mode"]: { $in: query.compilation_mode } } : {};
    const platformMatch = query.platform.length ? { [prefix + "metaData.platform"]: { $in: query.platform } } : {};
    const solutionMatch = query.solution.length ? { [prefix + "metaData.solution"]: { $in: query.solution } } : {};
    const toolNameMatch = query.tool_name.length ? { [prefix + "metaData.tool_name"]: { $in: query.tool_name } } : {};
    const statusMatch = query.status.length ? { [prefix + "status"]: { $in: query.status } } : {};
...
function testCaseMatchGenerator(query: any, prefix: string = "") {
    prefix = prefix ? prefix + "." : "";
    const testCaseIdMatch = query._id ? { [prefix + "_id"]: new mongoose.Types.ObjectId(query._id) } : {}
    const testCaseStatusMatch = query.status.length ? { [prefix + "status"]: { $in: query.status } } : {};
    return {
        ...testCaseIdMatch,
        ...testCaseStatusMatch
    }
...
function validationTagMatchGenerator(query: any, prefix: string = "") {
    prefix = prefix ? prefix + "." : "";
    const validationTagIdMatch = query._id ? { [prefix + "_id"]: new mongoose.Types.ObjectId(query._id) } : {}
    const validationTagNameMatch = query.name.length ? { [prefix + "metaData.name"]: { $in: query.name } } : {};
    const validationTagExecutablePathMatch = query.executable_path.length ? { [prefix + "metaData.metaData.Executable Path"]: { $in: query.executable_path } } : {};
    const validationTagStatusMatch = query.status.length ? { [prefix + "status"]: { $in: query.status } } : {};
    return {
...
function validationPointMatchGenerator(query: any, prefix: string = "") {
    prefix = prefix ? prefix + "." : "";
    const validationPointIdMatch = query._id ? { [prefix + "_id"]: new mongoose.Types.ObjectId(query._id) } : {}
    const validationPointStatusMatch = query.status.length ? { [prefix + "status"]: { $in: query.status } } : {};
    const validationPointMacMatch = query.mac.length ? { [prefix + "levels.mac"]: { $in: query.mac } } : {};
    const validationPointDirectionMatch = query.direction.length ? { [prefix + "levels.direction"]: { $in: query.direction } } : {};
    const validationPointPacketIdentifierMatch = query.packet_identifier.length ? { [prefix + "levels.packet_identifier"]: { $in: query.packet_identifier } } : {};
    return {
...
export async function testSuiteAggregationBuilder(req: express.Request) {
    ...
    const results = await AggregationWrapper.getInstance(testSuiteModel.aggregate())
        ...
        .match(validationTagMatchGenerator(req.body.validationTags, "testCaseRef.validationTagRefs"))
        .lookup("validationpoints", "testCaseRef.validationTagRefs.validationPointRefs", "_id")
        .unwind("testCaseRef.validationTagRefs.validationPointRefs")
        .match(validationPointMatchGenerator(req.body.validationPoints, "testCaseRef.validationTagRefs.validationPointRefs"))
        .group({
            _id: "$_id",
...
export async function testCaseAggregationBuilder(req: express.Request) {
     ...
     const results = await AggregationWrapper.getInstance(testCaseModel.aggregate())
        ...
        .match(validationPointMatchGenerator(req.body.validationPoints, "validationTagRefs.validationPointRefs"))
        .group({
            _id: "$_id",
            metaData: { $first: "$metaData" },
            parent: { $first: "$parent" },
...
export async function validationPointAggregationBuilder(req: express.Request) {
    ...
    const results = await AggregationWrapper.getInstance(validationPointModel.aggregate())
        .match(validationPointMatchGenerator(req.body.validationPoints))
        .lookup("validationtags", "parent.validationTag.id", "_id", "parent.validationTag")
        .match(validationTagMatchGenerator(req.body.validationTags, "parent.validationTag"))
...
export async function filtersBuilder() {
    const test_suite_filters = await AggregationWrapper.getInstance(testSuiteModel.aggregate())
            .group({
                _id: null,
            ...
            .project().getAggregation().exec()
        
        const test_case_filters = await AggregationWrapper.getInstance(testCaseModel.aggregate())
            .group({
                _id: null,
                status: { $addToSet: "$status" },
            })
        ...
        const validation_tag_filters = await AggregationWrapper.getInstance(validationTagModel.aggregate())
            .group({
                _id: null,
        ...
        const validation_point_filters = await AggregationWrapper.getInstance(validationPointModel.aggregate())
            .group({
                _id: null,
                status: { $addToSet: "$status" },
